...
from car_app.forms import CarModelForm,CommentForm,jobsekeerFrom
...
def jobseekrsview(request):
    if request.method == 'POST':
        form = jobsekeerFrom(request.POST)
        if form.is_valid():
            form.save()
            return redirect('profile')  # Redirect to the job listings page
    else:
         form = jobsekeerFrom()

    return render(request, 'car_app/job.html', {'form': form})
